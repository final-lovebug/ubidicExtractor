"""SPEC.md(§5 ①, test/ 리포 기준) — 문서 정규화·오프셋 계산.

`test/app/pipeline/normalize.py`에서 포팅. **`load_fixture_request`(fixtures
읽기, CLI 전용)는 빼고 옮겼다** — 프로덕션에서는 문서가 fixtures가 아니라
큐 메시지/HTTP 요청 바디로 들어온다(`app.schema.ExtractRequest`를 그대로
받는다).

**D-5 결정(test/SPEC.md §10 원문 유지).** `content`는 마크다운을 포함한 원문
그대로 취급한다 — 마크다운을 제거·정규화하지 않는다. 백엔드가 하이라이트할
때도 요청에 담긴 `content`와 같은 문자열 기준으로 렌더링하므로, 서버가
임의로 정규화하면 오프셋이 어긋난다.
...
from app.schema import ExtractRequest, Occurrence
