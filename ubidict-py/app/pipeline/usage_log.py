"""Gemini 호출 사용량을 기록·집계하는 인프라 (`test/app/pipeline/usage_log.py`에서 포팅).

로그는 로컬 JSON Lines 파일(`logs/llm_usage.jsonl`, 한 줄 = 한 호출)로도
append하고(로컬 디버깅·`usage-report` 스타일 집계용), **동시에 표준출력에도
같은 내용을 구조화 JSON 한 줄로 찍는다** — ECS의 `awslogs` 로그 드라이버가
표준출력을 그대로 CloudWatch Logs로 보내주므로, 컨테이너가 재시작돼 로컬
파일이 사라져도 CloudWatch에는 이력이 남는다(task.md 참고).
"""
...
def log_call(entry: LlmCallLog, log_path: Path = DEFAULT_LOG_PATH) -> None:
    ...
    with log_path.open("a", encoding="utf-8") as f:
        f.write(line + "\n")
    print(json.dumps({"logType": "llm_usage", **asdict(entry)}, ensure_ascii=False))
