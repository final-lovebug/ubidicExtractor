...
| 5 | HOMOGRAPH (LLM) | ⚠️ **막힘, 재확정** — G6("주문")이 4번 중 1번만 나옴(운). `gemini-3.5-flash-lite`는 오탐도 재발해 신뢰 불가 — §10 D-13, `gemini-3.1-flash-lite` 복귀 권장 |
| 6 | FastAPI 래핑 | ⬜ 미착수 |
...
## 5단계 — HOMOGRAPH ⚠️ 막힘 (재확정 — 팀 판단 필요)

**통과 조건 (SPEC.md §8-5)**: 골드셋 `G6`("주문")을 찾고 `senses`가 **정확히 2개**. **데모 핵심이라 필수.** `G7`(활성) `G14`(정산) `G15`(등급)는 참고 지표.

**실제 결과**: 7차 시도(`gemini-3.5-flash-lite` + `02_forbidden.md` 규칙 추가)에서 처음으로 `G6`("주문")이 정확한 senses 2개로 나와서 한때 "해결"로 기록했으나, **같은 프롬프트로 3번 더 불러본 결과 4번 중 1번(25%)만 성공**이었다(§10 D-13):
...
**결론**: `gemini-3.5-flash-lite`는 신뢰할 수 없다 — "주문"을 가끔 찾지만 그 대가로 이전엔 없던 오탐까지 다시 나온다. **`gemini-3.1-flash-lite`로 되돌리는 걸 권장한다**(5회 연속 정밀도 1.00·오탐 0 유지했던 이력). 다만 그 모델도 "주문"은 끝내 못 찾았다 — 총 11번(3.1 모델 7회 + 3.5 모델 4회) 시도 끝에 **안정적인 해법을 못 찾았다는 게 최종 결론**이다.

**이건 코드 버그가 아니라 팀이 결정할 문제다**:
- `gemini-3.1-flash-lite`로 되돌리고 "주문"은 사람이 리뷰 화면에서 수동으로 추가하는 쪽으로 받아들일지
- §8-5의 "필수" 조건을 완화할지 — 제품 설계 결정이라 에이전트가 못 바꾼다

**확인 절차 (팀이 방향을 정한 뒤)**:
1. `.env`의 `GEMINI_MODEL`을 `gemini-3.1-flash-lite`로 되돌릴지 결정
2. `out/result.json`에서 안정적으로 나오는 HOMOGRAPH 후보(정산·활성류)의 `senses`가 말이 되는지 확인
3. "주문" 없이 데모를 어떻게 할지 팀 전체와 상의 (TEST-CHECKLIST.md D 표)
